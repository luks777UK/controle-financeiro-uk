...
function renderVault(){
  ...
  if(!entries.length){
    ...
    if(window.BrazilVaultV72)window.BrazilVaultV72.render();
    return;
  }
...
function resetDepositDialogV404(){
  ...
  $("depositDateV734").value=currentLocalDate();
...
function editDepositV404(id){
  ...
  $("depositDateV734").value=localDateKeyV410(entry.date)||currentLocalDate();
  feedback("depositFeedbackV4","");
...
$("saveDeposit").onclick=async event=>{
  ...
  const selectedDate=$("depositDateV734").value||currentLocalDate();
...
/* =========================================================
   v7.3.0-beta.5 Vault & Brazil Box — Live FX + Wise Calibration
   ========================================================= */
(function(){
  "use strict";
  const B={
  ...
  B.num=v=>Number.isFinite(Number(v))?Number(v):0;
  B.moneyGBP=v=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"GBP"}).format(B.num(v));
  B.moneyBRL=v=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(B.num(v));
  B.monthKey=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;
  B.clock=date=>date?new Intl.DateTimeFormat("pt-BR",{hour:"2-digit",minute:"2-digit",second:"2-digit"}).format(date):"—";

  B.ensure=()=>{
    if(!state)return null;
    state.vaultEntries=Array.isArray(state.vaultEntries)?state.vaultEntries:[];
    state.brazilBox=state.brazilBox&&typeof state.brazilBox==="object"?state.brazilBox:{};
    state.brazilBox.transfers=Array.isArray(state.brazilBox.transfers)?state.brazilBox.transfers:[];
    state.brazilBox.emergencyTarget=Math.max(0,B.num(state.brazilBox.emergencyTarget));
  ...
  B.confirmTransfer=async()=>{
    ...
    B.render();
    if(window.FinancialPlannerV71)window.FinancialPlannerV71.render();
    await persist("Envio ao Brasil registrado");
  };
  ...
  B.bind=()=>{
    ...
    B.$("refreshBrazilRateV72")?.addEventListener("click",B.fetchRate);
...
const APP_VERSION="7.3.0-beta.5";
...
function openBillWalletEditV732(type){
  if(!state)return;
  const isCard=type==="card";
  const current=Math.max(0,Number(isCard?state.card:state.cash)||0);

  $("billWalletEditTypeV732").value=type;
  $("billWalletEditTitleV732").textContent=isCard?"Editar Cartão":"Editar Envelope";
  $("billWalletCurrentV732").textContent=money(current);
  $("billWalletNewValueV732").value=current.toFixed(2);
  $("billWalletEditNoteV732").value="";
  $("billWalletEditFeedbackV732").textContent="";
  updateBillWalletEditPreviewV732();

  try{$("billWalletEditDialogV732").showModal()}
  catch{$("billWalletEditDialogV732").setAttribute("open","")}
}

function updateBillWalletEditPreviewV732(){
  const type=$("billWalletEditTypeV732")?.value;
  if(!type||!state)return;

  const current=Math.max(0,Number(type==="card"?state.card:state.cash)||0);
  const next=Math.max(0,Number($("billWalletNewValueV732")?.value)||0);
  const diff=Math.round((next-current)*100)/100;
  const el=$("billWalletEditDifferenceV732");
  if(!el)return;

  if(Math.abs(diff)<0.005){
    el.className="wallet-edit-diff-v732 neutral";
    el.textContent="Nenhuma alteração";
  }else if(diff>0){
    el.className="wallet-edit-diff-v732 added";
    el.textContent=`Ajuste: +${money(diff)}`;
  }else{
    el.className="wallet-edit-diff-v732 removed";
    el.textContent=`Ajuste: −${money(Math.abs(diff))}`;
  }
}


function billSortDateV735(bill){
  const d=parseBillDate317?.(bill?.due);
  return d instanceof Date&&!Number.isNaN(d.getTime())?d:new Date("2999-12-31T12:00:00");
}

function reconcileBillReservationsV735(){
  if(!state)return {available:0,reserved:0};

  const available=Math.max(0,Number(state.cash||0))+Math.max(0,Number(state.card||0));
  const bills=(state.bills||[])
    .filter(b=>!b.completed&&!b.paid)
    .sort((a,b)=>{
      const da=billSortDateV735(a).getTime();
      const db=billSortDateV735(b).getTime();
  ...
  for(const bill of bills){
    ...
    const reserve=Math.min(amount,remaining);
    bill.reserved=Math.round(reserve*100)/100;
    remaining=Math.max(0,Math.round((remaining-reserve)*100)/100);
    reservedTotal+=bill.reserved;
  }

  return {
    available:Math.round(available*100)/100,
    reserved:Math.round(reservedTotal*100)/100,
    unallocated:Math.round(remaining*100)/100
  };
}

async function saveBillWalletEditV732(){
  const type=$("billWalletEditTypeV732")?.value;
  if(!["cash","card"].includes(type)||!state)return;

  const input=$("billWalletNewValueV732");
  const raw=Number(input?.value);
  if(!Number.isFinite(raw)||raw<0){
    $("billWalletEditFeedbackV732").textContent="Digite um saldo válido.";
    return;
  }
  ...
  if(Math.abs(diff)<0.005){
    try{$("billWalletEditDialogV732").close()}catch{}
    return;
  }
  ...
  try{
    if(type==="card")state.card=next;
    else state.cash=next;

    const reconciliation=reconcileBillReservationsV735();

    state.history=Array.isArray(state.history)?state.history:[];
    const label=type==="card"?"Cartão":"Envelope";
    const note=$("billWalletEditNoteV732")?.value.trim()||"Ajuste manual";

    state.history.push({
      id:crypto.randomUUID?.()||String(Date.now()),
      type:"bill_wallet_adjustment",
      text:`${label} ajustado · ${note}`,
      wallet:type,
      before:current,
      after:next,
      amount:Math.abs(diff),
      direction:diff>0?"added":"removed",
      reconciledReserved:reconciliation.reserved,
      reconciledAvailable:reconciliation.available,
      date:new Date().toISOString()
    });

    state.history.push({
      id:crypto.randomUUID?.()||String(Date.now()+1),
      type:"bill_reserve_reconciliation",
      text:"Reservas das Bills recalculadas",
      amount:reconciliation.reserved,
    ...
    try{$("billWalletEditDialogV732").close()}catch{}
    await persist(`${label} atualizado e reservas recalculadas`);
  }catch(error){
    state=backup;
    render();
    fatalDiagnostic313("Reconciliar reservas após editar saldo 7.3.5",error,{wallet:type});
    $("billWalletEditFeedbackV732").textContent="Não foi possível salvar e recalcular as reservas.";
  }
}

$("editBillEnvelopeV732")?.addEventListener("click",()=>openBillWalletEditV732("cash"));
$("editBillCardV732")?.addEventListener("click",()=>openBillWalletEditV732("card"));
$("billWalletNewValueV732")?.addEventListener("input",updateBillWalletEditPreviewV732);
$("saveBillWalletEditV732")?.addEventListener("click",saveBillWalletEditV732);
...
function renderBillActivityV4(){
  ...
  container.innerHTML=items.map(item=>{
    ...
    const walletAdjustment=type==="bill_wallet_adjustment";
    const added=isDeposit||type.includes("deposit")||lower.includes("adicionado")||(walletAdjustment&&item.direction==="added");
    const removed=type==="bill_payment"||type==="bill_deleted"||lower.includes("paga")||lower.includes("excluída")||lower.includes("retirada")||(walletAdjustment&&item.direction==="removed");
